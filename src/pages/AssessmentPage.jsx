...
export default function AssessmentPage({ profile, onLogout }) {
  ...
  async function loadAssessment() {
    ...
    if (error || !assess) { navigate('/'); return }
    setAssessment(assess)

    // Load the other person's name
    const otherId = profile.role === 'coach' ? assess.boxer_id : assess.coach_id
    if (otherId) {
  ...
  async function saveRatings(submit = false) {
    ...
    const { error: upsertError } = await supabase
      .from('ratings')
      .upsert(rows, { onConflict: 'assessment_id,skill' })

    if (upsertError) {
      // Fallback: delete then insert if upsert fails (e.g. no unique constraint yet)
